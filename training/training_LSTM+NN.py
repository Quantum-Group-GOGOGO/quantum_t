...
class TimeSeriesLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        # 5 个 LSTM 层，分别处理每对时间序列
        self.lstm_1 = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.lstm_2 = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.lstm_3 = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.lstm_4 = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.lstm_5 = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)

        # 全连接层，用于将 LSTM 输出和协变量结合
        self.fc = nn.Linear(hidden_size * 5 + 9, output_size)

    def forward(self, close_volume_pairs, other_data):
        # 分别处理每对时间序列
        lstm_outputs = []
        for i, (lstm, pair) in enumerate(zip([self.lstm_1, self.lstm_2, self.lstm_3, self.lstm_4, self.lstm_5], close_volume_pairs)):
            h0 = torch.zeros(self.num_layers, pair.size(0), self.hidden_size).to(pair.device)
            c0 = torch.zeros(self.num_layers, pair.size(0), self.hidden_size).to(pair.device)
            out, _ = lstm(pair, (h0, c0))
            lstm_outputs.append(out[:, -1, :])  # 获取最后一个时间步的输出

        # 将 LSTM 的输出和协变量结合
        lstm_outputs = torch.cat(lstm_outputs, dim=1)
        combined = torch.cat((lstm_outputs, other_data), dim=1)
...
if __name__ == "__main__":
    ...
    # 定义模型参数
    input_size = 2  # 每对时间序列有两个输入特征 (close 和 volume)
    hidden_size = 50
    num_layers = 2
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # 创建时间序列对的列表
            close_volume_pairs = [
                torch.cat((close_1, volume_1), dim=2),
                torch.cat((close_10, volume_10), dim=2),
                torch.cat((close_60, volume_60), dim=2),
                torch.cat((close_240, volume_240), dim=2),
                torch.cat((close_1380, volume_1380), dim=2)
            ]
            
            # 前向传播
            outputs = model(close_volume_pairs, other_data)
