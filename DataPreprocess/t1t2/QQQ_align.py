...
def QQQ_Align(column_name,NDX,QQQ,timepoint):
    ...
    date_start=QQQ['datetime'].iloc[0].date()
    date_end=QQQ['datetime'].iloc[-1].date()
    ...
    for day_i in range(day_num+1):
        ...
        if date_i.weekday() == 5 or date_i.weekday() == 6:
            continue
        while(QQQ_found==0 and QQQ_index<QQQ.shape[0] and QQQ['datetime'].iloc[QQQ_index].date()==date_i):
            if(QQQ['datetime'].iloc[QQQ_index].time()>=time_destination):
                QQQ_found=1
                QQQ_index=QQQ_index-1
            QQQ_index=QQQ_index+1
        while(NDX_found == 0 and NDX_index<NDX.shape[0] and NDX['datetime'].iloc[NDX_index].date()==date_i):
            if(NDX['datetime'].iloc[NDX_index].time()>=time_destination):
                NDX_found=1
                NDX_index=NDX_index-1
            NDX_index=NDX_index+1
        ...
        while(QQQ['datetime'].iloc[QQQ_index] != NDX['datetime'].iloc[NDX_index]):
            Print('Waring!')
            if(QQQ['datetime'].iloc[QQQ_index] > NDX['datetime'].iloc[NDX_index]):
                NDX_found=0
                time_destination=QQQ['date'].iloc[QQQ_index].time()
                while(QQQ['datetime'].iloc[QQQ_index] < NDX['datetime'].iloc[NDX_index] and NDX_found==0 and NDX_index<NDX.shape[0] and NDX['date'].iloc[NDX_index].date()==date_i):
                    NDX_index=NDX_index+1
                    if(NDX['datetime'].iloc[NDX_index].time() >= time_destination):
                        NDX_found=1
                        break
                ...
                QQQ_found=0
                time_destination=NDX['datetime'].iloc[NDX_index].time()
                while(NDX['datetime'].iloc[NDX_index] < QQQ['datetime'].iloc[QQQ_index] and QQQ_found==0 and QQQ_index<QQQ.shape[0] and QQQ['date'].iloc[QQQ_index].date()==date_i):
                    QQQ_index=QQQ_index+1
                    if(QQQ['datetime'].iloc[QQQ_index].time() >= time_destination):
                        QQQ_found=1
                        break
        ...
        k=NDX['close'].iloc[NDX_index]/QQQ['close'].iloc[QQQ_index]
        print('K=', k)
        newdata=pd.DataFrame({'datetime': [day_i],'K':[k],'time1': [NDX['datetime'].iloc[NDX_index]],'time2': [QQQ['datetime'].iloc[QQQ_index]],'QQQ_start': [int(QQQ_start)],'NDX_start': [int(NDX_start)]})
...
QQQ_Align("average",NDX,QQQ,'120000')
QQQ_Align("open",NDX,QQQ,'120000')
QQQ_Align("high",NDX,QQQ,'120000')
QQQ_Align("low",NDX,QQQ,'120000')
#Close mast be the last aligned
QQQ_Align("close",NDX,QQQ,'120000')
#QQQ.pd
print(NDX.head())
print(QQQ.head())
print(QQQ.tail())
